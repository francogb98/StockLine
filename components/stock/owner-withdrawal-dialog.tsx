...
export function OwnerWithdrawalDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(qty) || qty <= 0 || qty > product.stock) {
      setError("La cantidad debe ser mayor a cero y no superar el stock actual");
      return;
    }
    ...
    try {
      const result = await onConfirm(qty, reason.trim());
      if (result.ok) {
        setQuantity("");
        setReason("");
        onClose();
      } else {
        setError(result.error ?? "Error al registrar retiro");
      }
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative z-10 flex w-full max-w-md flex-col rounded-lg bg-card shadow-xl">
        ...
        >
          ...
          <div>
            <label htmlFor="withdrawal-qty" className="block text-sm font-medium text-foreground">
              Cantidad
            </label>
            <p className="text-xs text-muted-foreground mb-1">
              Usá un valor positivo. El stock disminuirá en esa cantidad.
            </p>
            <input
              id="withdrawal-qty"
              type="number"
          ...
          <div>
            <label htmlFor="withdrawal-reason" className="block text-sm font-medium text-foreground">
            ...
            <input
              ...
              placeholder="Ej: Retiro para uso personal"
              className={cn(
                "mt-1 h-9 w-full rounded-md border bg-background px-3 text-sm",
                "focus:border-primary focus:outline-none focus:ring-2 focus:ring-primary/20",
              )}
